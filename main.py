...
def webhook():
    ...
    if arv and repair_cost:
        try:
            ...
            offer_note = f"Start at ${min_offer}, negotiate up to ${max_offer}."
        except:
            offer_note = "Offer calculation failed."
...
"""
    ...
    updated_fields = {
        ...
        "min_offer_amount": round(min_offer, 2) if min_offer else seller_data.get("min_offer_amount"),
        "max_offer_amount": round(max_offer, 2) if max_offer else seller_data.get("max_offer_amount"),
        "verbal_offer_amount": round(verbal_offer, 2) if verbal_offer else seller_data.get("verbal_offer_amount")
    }

    for field in [
        "asking_price", "repair_cost", "estimated_arv", "follow_up_date", "follow_up_reason", "follow_up_set_by",
        "property_address", "condition_notes", "lead_source", "bedrooms", "bathrooms", "square_footage",
        "year_built", "conversation_stage"
    ]:
        if data.get(field) is not None:
            updated_fields[field] = data[field]
        elif seller_data.get(field) is not None:
            updated_fields[field] = seller_data[field]

    update_seller_memory(phone_number, updated_fields)
